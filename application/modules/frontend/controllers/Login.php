...
//************************* service on functionalities****************************//
	...
	public function serviceOnlogin()
	{
		$this->template->set ( 'page', 'ServiceOn Login' );
	...
	public function serviceOnverifyotp()
	{
		$this->template->set ( 'page', 'ServiceOn OTP Verification' );
	    $this->template->set ( 'description', '' );
	    $this->template->set ( 'keywords', '' );
	    $this->template->set_theme('default_theme');
	    $this->template->set_layout ('default')
	    ...
	    ->set_partial ( 'header', 'partials/header' )
	    ->set_partial ( 'footer', 'partials/footer' );
	    $this->template->build ('verify-otp');
	}
	...
	//	print_r($this->input->post());
		...
		if($user['status'] == 1){
			if(empty($otp['id'])){
				$data['my_ref_code'] = strtoupper(base_convert(strtotime(date('Y-m-d H:i:s')), 10, 36)).strtoupper(base_convert($user['id'], 10, 36)) ;
				$data['id'] = $user['id'];
			...
			$this->session->set_userdata ( 'olouserid', $user['id'] );
			$this->session->set_userdata ( 'olousername', $user['name'] );
			$this->session->set_userdata ( 'olouserlname', $user['lname'] );
			$this->session->set_userdata ( 'olouseremail', $user['email'] );
			$this->session->set_userdata ( 'olousermobile', $user['mobile'] );
			$data['status'] = 1;
		}else{
			$data['status'] = 0;
		}
	...
	public function logout() {
		...
		$this->session->unset_userdata ( 'olouserlname' );
