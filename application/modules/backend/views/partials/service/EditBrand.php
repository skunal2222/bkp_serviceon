<form id="editcategory" name="editcategory" action="" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="id" value="<?php echo $brands[0]['id'];?>"/>
								...
								<div class="col-md-6">
										<div class="form-group" id="error-name">
											...
											<select name="category_id" id="category_id" class="form-control">
													...
													<option value="<?php echo $category['id'];?>" <?php if($brands[0]['category_id'] == $category['id']) {?>selected<?php }?>><?php echo $category['name'];?></option>
													<?php } ?>
												</select>
										</div>
									...
									<div class="col-md-6">
										<div class="form-group" id="error-name">
											...
											<input type="text" class="form-control" value="<?php echo $brands[0]['name'];?>" id="name" name="name" />
										</div>
										<div class="messageContainer"></div>
									</div>
								...
								<div class="row">
	                                <div class="col-md-6">
		                              	<div class="form-group" id="error-image">
	                                       	...
	                                       	<input type="file" value="" name="image" id="image" value="<?php echo $brands[0]['image'];?>" class="form-control " >
										</div>
										<span>
											<img src="<?php echo asset_url();?><?php echo $brands[0]['image'];?>" width="160px" height="80px" />
										</span>
										...
										<div class="form-group" id="error-name">
											...
											<select id="status" class="form-control" name="status">
												<option value="1" <?php if(isset($brands[0]['status']) && $brands[0]['status'] == 1) {?>selected<?php }?>>Enable</option>
												<option value="0" <?php if(isset($brands[0]['status']) && $brands[0]['status'] == 0) {?>selected<?php }?>>Disable</option>
											</select>
										</div>
										<div class="messageContainer"></div>
									</div>
