<form id="editcategory" name="editcategory" action="" method="post"
	enctype="multipart/form-data">
		<?php //print_r($categories);?>
		<input type="hidden" name="id"
		value="<?php echo $services[0]['id'];?>" />
								...
								<div class="form-group" id="error-name">
									...
									<select name="category_id" id="category_id"
										...
										<option value="">Select Vehical Type</option>
											...
											<?php if($services[0]['category_id'] == $category['id']) {?>
								...
								<div class="form-group" id="error-name">
									...
									<select name="brand_id" id="brand_id" class="form-control">
										<option value="">Select Brand</option>
											...
											<?php if($services[0]['brand_id'] == $brand['id']) {?>
											selected <?php }?>><?php echo $brand['name'];?></option>
													<?php endforeach;?>														
													</select>
								</div>
								<div class="messageContainer"></div>
								...
								<div class="form-group" id="error-name">
									...
									<select name="model_id" id="model_id" class="form-control">
										<option value="">Select Model</option>	
											...
											<?php if($services[0]['model_id'] == $model['id']) {?>
											selected <?php }?>><?php echo $model['name'];?></option>
													<?php endforeach;?>
												</select>
								</div>
								<div class="messageContainer"></div>
									...
									<label class="control-label">Service Type<span
										...
										<option value="">Select Service Type</option>	
													<?php foreach ($subcategories as $subcategory) : ?>
													 <option value="<?php echo $subcategory['id'];?>" <?php if($services[0]['subcategory_id'] == $subcategory['id']) {?>selected<?php }?>><?php echo $subcategory['name'];?></option>
													<?php endforeach;?>
													</select>
								</div>
								<div class="messageContainer"></div>
								...
								<div class="form-group" id="error-name">
									...
									<select name="catsubcat_id" id="catsubcat_id" class="form-control">
										...
										<?php foreach ($servicegroup as $value) : ?>
											<option value="<?= $value['id'] ?>" <?= ($services[0]['catsubcat_id'] == $value['id'])? "selected":""?>><?= $value['name'] ?></option>
										<?php endforeach; ?>
									</select>
								</div>
								<div class="messageContainer"></div>
							...
							<div class="col-md-6">
								<div class="form-group" id="error-name">
									<label class="control-label">Service Name <span
										...
										<input type="text" class="form-control" value="<?php echo $services[0]['name'];?>" id="name" name="name" />
								</div>
								<div class="messageContainer"></div>
							</div>
						</div>
							...
							<div class="col-md-6">
								<div class="form-group" id="error-image">
									...
									<input type="file"
										...
										value="<?php echo $services[0]['service_icon'];?>"
										class="form-control ">
								</div>
								<span> <img
									src="<?php echo asset_url();?><?php echo $services[0]['service_icon'];?>"
							...
							<div class="col-md-6">
								<div class="form-group" id="error-name">
									...
									<select id="status" class="form-control" name="status">
										<option value="1"
											<?php if(isset($services[0]['status']) && $services[0]['status'] == 1) {?>
											selected <?php }?>>Enable</option>
										<option value="0"
											<?php if(isset($services[0]['status']) && $services[0]['status'] == 0) {?>
											selected <?php }?>>Disable</option>
									</select>
								</div>
								<div class="messageContainer"></div>
							</div>
							...
							<div class="col-md-6">
								<div class="form-group" id="error-name">
									...
									<input type="text" class="form-control" id="price" name="price"
										value="<?php echo $services[0]['price'];?>" />
								</div>
								<div class="messageContainer"></div>
							</div>
							<div class="col-md-6">
