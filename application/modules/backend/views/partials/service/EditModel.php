<form id="editcategory" name="editcategory" action="" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="id" value="<?php echo $models[0]['id'];?>"/>
									...
									<div class="col-md-6">
										<div class="form-group" id="error-name">
											...
											<select name="category_id" id="category_id" class="form-control">
													...
													<option value="<?php echo $category['id'];?>" <?php if($models[0]['category_id'] == $category['id']) {?>selected<?php }?>><?php echo $category['name'];?></option>
													<?php } ?>
												</select>
										</div>
										...
										<div class="form-group" id="error-name">
											...
											<select name="brand_id" id="brand_id" class="form-control">
													...
													<option value="<?php echo $brand['id'];?>" <?php if($models[0]['brand_id'] == $brand['id']) {?>selected<?php }?>><?php echo $brand['name'];?></option>
													<?php endforeach;?>
												</select>
										</div>
										<div class="messageContainer"></div>
								...
								<div class="row">
									...
									<div class="col-md-6">
										<div class="form-group" id="error-name">
											...
											<input type="text" class="form-control" value="<?php echo $models[0]['name'];?>" id="name" name="name" />
										</div>
										<div class="messageContainer"></div>
									</div>
										<div class="col-md-6">
										<div class="form-group" id="error-name">
											...
											<select id="status" class="form-control" name="status">
												<option value="1" <?php if(isset($models[0]['status']) && $models[0]['status'] == 1) {?>selected<?php }?>>Enable</option>
												<option value="0" <?php if(isset($models[0]['status']) && $models[0]['status'] == 0) {?>selected<?php }?>>Disable</option>
											</select>
										</div>
										<div class="messageContainer"></div>
									</div>
...
$('#editcategory').bootstrapValidator({
    ...
    fields: {
        ...
        category_id: {
            validators: {
                notEmpty: {
                    message: 'Vehical Type is required and cannot be empty'
                }
            }
        },
       brand_id: {
...
function updateCategory() {
	var options = {
	 		...
	 		url : base_url+'admin/model/update',
	 		semantic : true,
	 		dataType : 'json'
	 	};
