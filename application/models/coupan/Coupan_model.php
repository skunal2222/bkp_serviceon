...
class Coupan_model extends CI_Model {
	...
	public function getVendorById($a) {
		$this->db->select ('garage_name')->from ( TABLES::$VENDOR . ' AS a' )->where ( 'id', $a );		
		$query = $this->db->get ();
		$result = $query->row_array ('garage_name');
	...
	public function getAllVendorsList() {
		$this->db->select ( 'id, garage_name' )->from ( TABLES::$VENDOR );
		$query = $this->db->get ();
		$result = $query->result_array ();
		return $result;
	}
	...
	public function addCoupon($coupon) {
        ...
        $this->db->select('id')->from(TABLES::$TBL_COUPON)->where ( $params ); 
        $query = $this->db->get();
        $result = $query->result_array();  
        if($coupon ['coupon_code']==''){
        		$this->db->insert ( TABLES::$TBL_COUPON, $coupon );
				$data ['status'] = 1;
				$data ['msg'] = "Coupon Added successfully";
        	...
        	if (count($result) <= 0) { 
				...
				$data ['status'] = 0;
	            $data ['msg'] = "Coupon code already exists."; 
	            return $data;
	        }
        }
	...
	function fetch_couponData_With_code($tablename,$codename,$coupon_code){
		$this->db->select("*");
		$this->db->from($tablename);
		$this->db->where($codename,$coupon_code);
		$query = $this->db->get();
		$result = $query->result();
