...
class Mechanic_model extends CI_Model {
	...
	public function ongoingOrders($params) {
		...
		if($params['role'] == 1){
			$where = '(a.vendor_id = '.$params['user_id'].' and (a.status = "2" or a.status="3"))';
		}else{
	...
	public function updatePassword ($data) {
		...
		if(empty($user))
		{
			$user = $this->db->get_where(TABLES::$EMPLOYEE,array('mobile' => $data ['mobile']))->result_array();
			$this->db->where ( 'mobile', $data['mobile'] );
	...
	public function addUserTracking($param) {
		$user_id = $param['user_id'];		
		$track_data = json_decode($param['data'],true);	
		
		
		foreach ($track_data as $key => $jsons) {			
			
			$data1 = array();
			$data1['user_id'] = $user_id;
			$data1['role_id'] = $jsons['user_role'];
			$data1['latitude'] = $jsons['latitude'];
			$data1['longitude'] = $jsons['longitude'];
			$data1['distance'] = $jsons['distance'];
			$data1['created_date_time'] = date("Y-m-d H:i:s");
			$data1['packet_date_time'] = $jsons['date_time'];
			
			$insert = $this->db->insert(TABLES::$TBL_USER_TRACKING,$data1);		
			$result = $this->db->insert_id();
