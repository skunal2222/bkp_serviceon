...
class Profitmodel extends CI_Model {
    ...
    return $this->db->select('tb.orderid, tb.vendor_id, tb.amount_received, tv.garage_name, tv.name, vbc.min_amount, vbf.value, vbc.billing_cycle, vbc.commission_spare, vbc.commission_service,vbc.with_service_tax')
                        ...
                        ->where('tb.status', 7)
                        ->where('tb.pay_mode', 1)
                        ->where('vbf.billing_field', 'gateway_effective_date')
    ...
    function get_all_spare($param) {
        $data = $this->db->select('ROUND(SUM(total_amount), 0) as total')
    ...
    function get_all_service($param, $with_service_tax) {
        $result = array(
            'company_tax_value' => 0,
            'total' => 0,
            'vendor_tax_value' => 0
        );
        $data = $this->db->select('service_price, tax')
                        ->from('tbl_booking_services')
                        ->where('service', '1')
                        ->where('is_checked', '1')
                        ->where_in('orderid', $param)
        ...
        if(!empty($data)) {
            foreach ($data as $value) {
                if($with_service_tax == 1) {
                    $result['company_tax_value'] = round($result['company_tax_value'] + (($value['tax']/2) / 100) * $value['service_price']);
                    $result['total'] = $result['total'] + $value['service_price'];
                    $result['vendor_tax_value'] = round($result['vendor_tax_value'] + (($value['tax']/2) / 100) * $value['service_price']);
                } else {
                    $result['company_tax_value'] = round($result['company_tax_value'] + ($value['tax'] / 100) * $value['service_price']);
                    $result['total'] = $result['total'] + $value['service_price'];
