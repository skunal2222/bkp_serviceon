...
class Mechanic_model extends CI_Model {
	...
	public function updateVehicleALLDocument($cat){  
	...
	public function getallPackageByids($data)
	{

 			   if (!empty($data['subcategory_id'])) {
 			   		$this->db->join('subcategory as ps','p.subcategory_id = ps.id');
					$this->db->where('ps.sub_id',$data['subcategory_id']);
 			   }
				 ...
				 if (!empty($data['model_id'])) {
	 				...
	 				$this->db->join('package_models as pm','pm.package_id = p.id');
	 				$this->db->where('pm.model_id IN('.$data['model_id'].')');
				 }

				  
				 if (!empty($data['service_id'])) { 
			 	 	$this->db->join('package_services as pq','pq.package_id = p.id');
			 		$this->db->where('pq.id IN('.$data['service_id'].')');
				 } 
