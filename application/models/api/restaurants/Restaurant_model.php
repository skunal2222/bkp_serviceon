...
class Restaurant_model extends CI_Model {
	...
	public function getGarage($map)
	{
		...
		$this->db->select ( 'a.id,a.radius, a.garage_name,a.mobile, IFNULL( 6371000 * acos( cos( radians(' . $map ['latitude'] . ') ) * cos( radians( a.latitude ) ) * cos( radians( a.longitude ) - radians(' . $map ['longitude'] . ') ) + sin( radians(' . $map ['latitude'] . ') ) * sin( radians( a.latitude ) ) ) ,0) AS distance' )->from ( TABLES::$RESTAURANT . ' AS a' );
		$this->db->where ( 'a.category_id', $map ['category_id'] );
	...
	public function getSearchedVendor($map) {
		...
		$this->db->select ( 'a.id, a.radius, a.garage_name, a.landmark, a.description, a.email, a.mobile, a.image, a.rating, c.coupon_code, a.latitude, a.longitude, IFNULL( 6371 * acos( cos( radians(' . $map ['latitude'] . ') ) * cos( radians( a.latitude ) ) * cos( radians( a.longitude ) - radians(' . $map ['longitude'] . ') ) + sin( radians(' . $map ['latitude'] . ') ) * sin( radians( a.latitude ) ) ) ,0) AS distance' )->from ( TABLES::$RESTAURANT . ' AS a' );
		$this->db->join(TABLES::$TBL_COUPON. ' AS c','c.garage_id=a.id','left');
		$this->db->where ( 'a.status', 1 );
		if(array_key_exists("searchKey",$map)){
			$this->db->like('a.garage_name',$map['searchKey'],'both');
        } else {
	...
	{
		$this->db->select ( 'a.id, a.radius, a.garage_name, a.landmark, a.locality, a.address, a.pincode, a.description, a.email, a.mobile, a.image, a.rating, tc.coupon_code, IFNULL( 6371 * acos( cos( radians(' . $map ['latitude'] . ') ) * cos( radians( a.latitude ) ) * cos( radians( a.longitude ) - radians(' . $map ['longitude'] . ') ) + sin( radians(' . $map ['latitude'] . ') ) * sin( radians( a.latitude ) ) ) ,0) AS distance, b.id as service_id, b.name as service_name, b.catsubcat_id as service_category, c.id as spare_id, c.name as spare_name, c.catsubcat_id as spare_category' );
		...
		$this->db->join(TABLES::$TBL_COUPON. ' AS tc','tc.garage_id=a.id','left');
