...
class PayUMoney{
	...
	public function __construct(){
		...
		$this->merchant_id = 'xxxxxxx';
		$this->submerchant_id = 'xxxxxxxx';
		$this->salt = 'xxxxxx';
		$this->response_url = base_url().'order/payment/response';
		$this->paymode = 9;
		$this->provider = 'payu_paisa';
	}
	...
	public function payNow(array $details){
		...
		$post = array('key'=>$this->merchant_id,
				...
				'merchantid'=>$this->submerchant_id,
				'salt'=>$this->salt
		);
	...
	function checkValidPayment($params) {
		...
		error_log("Hash Seq:".$retHashSeq,0);
	...
	function getHashes($params)
	{
		...
		return array('result'=>$paymentHash,'status'=>1);
	}
