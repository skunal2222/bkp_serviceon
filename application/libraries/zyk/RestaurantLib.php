...
public function addRestaurant($map){
	...
	public function updateVendor($map){
	    ...
	    $this->sendVendorBookingEmail($restaurant['userdata']);
	...
	public function sendVendorBookingEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Vendor Add |'. COMPANY;
		$this->CI->pkemail->subject = 'Vendor Added Successfully.';
		$this->CI->pkemail->mctag = 'Vendor Add';
		$this->CI->pkemail->attachment = 0;
		$this->CI->pkemail->to = $data ['email'];
		$this->CI->template->set ( 'data', $data );
		$this->CI->template->set ( 'page', 'vendor' );
		$this->CI->template->set_layout ( false );
		$text_body = $this->CI->template->build ( 'frontend/emails/vendor', '', true );
	...
	public function garageDetailsByVendorID2($value=NULL)
	{
		$this->CI->load->model ( 'restaurants/Restaurant_model', 'restaurant' );
		return $this->CI->restaurant->garageDetailsByVendorID2 ( $value );	
	}
	...
	public function getCategoriesByVendorId($vendor_id)
	{
		$this->CI->load->model ( 'restaurants/Restaurant_model', 'restaurant' );
		return $this->CI->restaurant->getCategoriesByVendorId ( $vendor_id );
	}
	
	public function getBrandByVendorId($vendor_id)
	{
		$this->CI->load->model ( 'restaurants/Restaurant_model', 'restaurant' );
		return $this->CI->restaurant->getBrandByVendorId ( $vendor_id );
	}

	public function getModelByVendorId($vendor_id)
	{
		$this->CI->load->model ( 'restaurants/Restaurant_model', 'restaurant' );
		return $this->CI->restaurant->getModelByVendorId ( $vendor_id );
	}
	// dev - kunal
}	
